...
local function readJobID()
    ...
    if success and response then
        ...
        if successDecode and data and data.job_id and data.job_id ~= "" then
            return data.job_id:gsub("%s+", "") -- remove espaços
        end
    end
...
task.spawn(function()
    ...
    while true do
        ...
        if newJob then
            erroCount = 0 -- reset contador de erros
            ...
            if newJob ~= lastJob then
                ...
                if gui then
                    if setJobIDText(gui, newJob) then
                        wait(0.1) -- delay anti-detecção
                        clickJoinButton(gui)
                    else
                        prints("❌ Campo de texto não encontrado.")
                    end
                else
                    prints("❌ UI não encontrada no momento do novo JobID.")
                end
            end
        else
            erroCount += 1
            if erroCount % 5 == 0 then -- só mostra erro a cada 5 falhas seguidas
                prints("⚠️ Erro ao buscar JobID do site. Tentando novamente...")
            end
        end

        wait(1) -- espera 1 segundo entre requisições
    end
end)
